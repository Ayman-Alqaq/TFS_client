...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if not hasPlayer then
		player:sendCancelMessage('Use on Silencer Plateau, located in the south-eastern part of Roshamuul.')
		return true
	end
	if hasMonsters then
		player:sendCancelMessage('You need to kill all monsters in the area.')
		return true
	end
	...
	if Game.getStorageValue(config.storage) <= 0 then
		...
		player:sendCancelMessage('Please try again later.')
	end	
end