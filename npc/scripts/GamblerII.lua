...
local config = {
	...
	dicerCounter = Position(npcPosition.x + 1, npcPosition.y - 1, npcPosition.z)
}
...
local function getBetValue(cid, position)
    local value = 0
    ...
    if tile then
        ...
        local tempMoney = {}
        for _, item in pairs(Tile(position):getItems()) do
            if table.contains({2160, 2152, 2148}, item:getId()) then
                value = value + getCoinValue(Tile(position):getItems()) * item:getCount()
...
npcHandler:setMessage(MESSAGE_GREET, "Welcome to my casino! I'm offering {high/low} and {numbers}, to start just put your {money} on {counter} and say {number} or {high/low}.")
