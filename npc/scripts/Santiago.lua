...
local function greetCallback(cid)
	...
	if player:getStorageValue(Storage.RookgaardTutorialIsland.SantiagoNpcGreetStorage) < 1 then
		npcHandler:setMessage(MESSAGE_GREET, "Hello |PLAYERNAME|, nice to see you! I saw you walking by and wondered if you could help me. Could you? Please, say {yes}! If you would like to skip this tutorial, say {skip tutorial}. You may also skip rookgaard and head straight to the oracle by saying {skip rookgaard}.")
		storeTalkCid[cid] = 0
	elseif player:getStorageValue(Storage.RookgaardTutorialIsland.SantiagoNpcGreetStorage) == 1 then
		npcHandler:setMessage(MESSAGE_GREET, "Oh, |PLAYERNAME|, it's you again! It's probably impolite to disturb a busy adventurer like you, but I really need help. Please, say {yes}! If you would like to skip this tutorial, say {skip tutorial}. You may also skip rookgaard and head straight to the oracle by saying (skip rookgaard).")
		storeTalkCid[cid] = 0
...
local function creatureSayCallback(cid, type, msg)
	...
	local player = Player(cid)
	if msgcontains(msg, "skip tutorial") then
		local destination1 = Position(32097, 32219, 7)
		player:teleportTo(destination1)
		player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
		destination1:sendMagicEffect(CONST_ME_TELEPORT)
		player:addItem(2383,1)
		player:addItem(2464,1)
		player:addItem(2120,1)
		player:addItem(2554,1)
	elseif isInArray({"skip rookgaard", "skip rook"}, msg) then
		player:addExperience(4200, true)
		local destination2 = Position(32104, 32191, 6)
		player:teleportTo(destination2)
		player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
		destination2:sendMagicEffect(CONST_ME_TELEPORT)
		player:addItem(2464,1)
		player:addItem(2383,1)
	elseif isInArray({"yes", "right", "ok"}, msg) then
		...
		if storeTalkCid[cid] == 0 then
			...
			Position(32573, 32543, 6):sendMagicEffect(CONST_ME_TUTORIALARROW)
			storeTalkCid[cid] = 1
		elseif storeTalkCid[cid] == 1 then
			npcHandler:say("Alright! Do you see the button with the book on the right hand side of the client? If you hover over it, you should see a pop-up window which says 'Open quest log'? There you can check the status of quests, like this one. {Bye} for now!", cid)
		...
		elseif storeTalkCid[cid] == 2 then
			if player:getItemCount(2464) > 0 then
				...
				player:addItem(2464, 1)
				npcHandler:say("Oh no, did you lose my armor? Well, lucky you, I have a spare one here. Don't lose it again! Now we're getting to the fun part, let's get you armed! Are you ready for some {action}?", cid)
				storeTalkCid[cid] = 3
			end
		elseif storeTalkCid[cid] == 3 then
			...
			Position(32576, 32543, 6):sendMagicEffect(CONST_ME_TUTORIALARROW)
			player:addItem(2398, 1)
			npcHandler:releaseFocus(cid)
			npcHandler:resetNpc(cid)
		...
		elseif storeTalkCid[cid] == 9 then
			...
			player:addMapMark(Position(32595, 32536, 7), MAPMARK_GREENSOUTH, "To Zirella")
			npcHandler:releaseFocus(cid)
			npcHandler:resetNpc(cid)
		end
