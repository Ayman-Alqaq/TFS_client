...
local function greetCallback(cid)
	...
	elseif player:getStorageValue(Storage.RookgaardTutorialIsland.ZirellaNpcGreetStorage) == 8 then
		npcHandler:setMessage(MESSAGE_GREET, "Oh, welcome back! Are you here for a little chat? Just use the highlighted {keywords} again to choose a {topic}.")
	end
	return true
end
...
local function creatureSayCallback(cid, type, msg)
	...
	if isInArray({"yes", "quest", "ok"}, msg) then
		...
		elseif storeTalkCid[cid] == 8 then
			...
			Position(32599, 32531, 7):sendMagicEffect(CONST_ME_TUTORIALARROW)
