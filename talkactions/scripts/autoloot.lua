function onSay(player, words, param)
	...
	if action == "add" then
		...
		if not item then
		player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Invalid Parameters. '!autoloot add, {item name or ID}'")
		return false
		end
		...
		if itemType:getId() == 0 then
			...
			if itemType:getId() == 0 then
				player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "There is no item with that ID/name. Please double check the item name/ID.")
				return false
			end
		end
		...
		for i = AUTOLOOT_STORAGE_START, AUTOLOOT_STORAGE_END do
			...
			if size == AUTO_LOOT_MAX_ITEMS then
				player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "The list is full, please remove items from the list to make some room.")
				break
			end
	...
	elseif action == "remove" then
		...
		if itemType:getId() == 0 then
			itemType = ItemType(tonumber(item))
			if itemType:getId() == 0 then
				player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "There is no item with that ID/name.")
				return false
			end
		end
	...
	elseif action == "clear" then
		...
		player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Use the commands: \n!autoloot add, item name/ID\n!autoloot remove, item name/ID\n!autoloot show\n!autoloot clear")
	end
